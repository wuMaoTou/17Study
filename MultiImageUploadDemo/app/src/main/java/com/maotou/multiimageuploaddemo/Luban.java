...
/**
 * Created by wuchundu on 17-5-23.
 * 鲁班算法图片压缩
 */
public class Luban {
     ...
     *
     * @see #getPhotoCacheDir(String)
     */
    private static synchronized File getPhotoCacheDir() {
        return getPhotoCacheDir(Luban.DEFAULT_DISK_CACHE_DIR);
    }
     ...
     * @return
     */
    public static Luban getInstant() {
        if (INSTANCE == null) {
          INSTANCE = new Luban(Luban.getPhotoCacheDir());
        }
        return INSTANCE;
    }
    ...
    public Flowable<File> asObservable() {
        if (gear == FIRST_GEAR) {
          return Flowable.just(mFile).map(new Function<File, File>() {
            @Override
            public File apply(@io.reactivex.annotations.NonNull File file) throws Exception {
              return firstCompress(file);
            }
